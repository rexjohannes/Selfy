...
public class ToggleCommand extends Command {
    public ToggleCommand(Selfy selfy) {
        super(selfy, "toggle", "Toggle a module", Category.UTIL);
        this.aliases.add("t");
    }
    ...
    public void onCommand(String[] args, MessageCreateEvent event) {
        ...
        Module m = selfy.moduleUtils.getModuleByName(args[0]);
        if(m == null) {
            MessageUtils.editMessage("Error", "Module not found", Color.RED.getRGB(), event.getMessage());
            return;
        }
        if(!m.canBeUsed(selfy.accountType)) {
